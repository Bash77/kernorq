...
def test_pipeline_successful_first_attempt_no_recovery():
    ...
    try:
        ...
        assert "has('RECOVERY_STARTED')" in js or "RECOVERY_STARTED" in js
    finally:
        try: store.close()
        except: pass
        _safe_unlink(db)
...
def test_verification_recovery_human_readable():
    ...
    assert "View verification evidence" in js
    assert "Verification succeeded" in js or "✓ Verification" in js
    assert "View raw recovery history" in js or "View raw" in js

    # Recovery summary for no recovery
    assert "None required" in js or "Recovery: None" in js
